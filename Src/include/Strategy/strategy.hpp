...
#include "./../Context/workload.hpp"
#include "./../Runtime/hardware_context.hpp"
...
public:
    ...
    inline vector<strategy>
    get_strategies(HardwareContext &hrd)
    {
    ...
    inline vector<string> get_strategy_names(vector<strategy> &strategies)
    {
        ...
        for (const auto &s : strategies)
        {
            if (!s.is_available)
                continue;
            string name = string(magic_enum::enum_name(s.device));
